...
import SetTodoAsDoneUseCase from "../../../usecases/todos/set-todo-as-done-usecase.interface"
...
export default class SetTodoAsDoneControllerImplementation implements SetTodoAsDoneController {
  private readonly errorMessage = "[SetTodoAsDoneController] execute"

  constructor(
    private readonly todoValidator: TodoValidator,
    private readonly setTodoAsDoneUseCase: SetTodoAsDoneUseCase
  ) {}
  ...
  public async execute(request: AdaptedRequest<null>): Promise<ControllerResponse<undefined>> {
    ...
    try {
      ...
      await this.setTodoAsDoneUseCase.execute(userId, todoId)
      return { status: 204 }
    } catch (err) {
