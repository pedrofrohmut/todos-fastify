...
import SetTodoAsNotDoneControllerImplementation from "../../../../../src/domain/controllers/todos/implementations/set-todo-as-not-done.controller"
...
const setTodoAsNotDoneController = new SetTodoAsNotDoneControllerImplementation(
  todoValidator,
  setTodoAsNotDoneUseCase
)
...
describe("SetTodoAsNotDoneControllerImplementation", () => {
  test("Null params => 400/message", async () => {
    request.params = null
    // Given
    expect(request.params).toBeNull()
    // When
    const controllerResponse = await setTodoAsNotDoneController.execute(request)
    // Then
  ...
  test("Valid request. But user not found => 400/message", async () => {
    const { connection, foundUser, setTodoAsNotDoneController } = await setupUserNotFound()
    // Given
  ...
  test("Valid request and user found. But todo not found => 400/message", async () => {
    const { connection, foundUser, foundTodo, setTodoAsNotDoneController } =
...
async function setupUserAndTodoFound() {
  ...
  return { connection, foundUser, foundTodo, setTodoAsNotDoneController }
}
...
async function setupUserNotFound() {
  ...
  const foundUser = await findUserByIdService.execute(userId)
  return { connection, foundUser, setTodoAsNotDoneController }
}
